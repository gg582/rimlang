...
|---|---|---|
| `림... <내용>` | **표준 출력**: 림의 대사를 터미널에 출력 | `림... 오늘의 퀴즈를 시작하겠습니다!` |
| `흡...` | **표준 입력**: 사용자로부터 한 줄을 입력받아 변수에 바인딩 | `흡...` |

### 3. 결정론적 연산 및 튜링 완전 OISC
| 문법 패턴 | 분류 | 설명 및 동작 |
|---|---|---|
...
| `X의 반대말은?` | 단항 함수 | NOT 부정 연산 (`통모짜핫도그` $\rightarrow$ `요즘잘자쿨냥이`) |
| `이제 ... 있으니까 ...` | 상태 판정 | 실시간 지시어(`이제`) & 불리언 축약자(`.무`) |
...
│   └── runtime/        # 튜링 완전 OISC VM, 블록 가드 및 I/O 런타임
├── examples/           # 공식 및 창작 림언어 소스 (.rim)
│   ├── trickcal_rim_sum_squares.rim  # 예제 1: 원작 100% 반영
│   ├── creative_rim_jokes.rim        # 예제 2: 창작 썰렁개그 및 방언 호환
│   ├── turing_io_llm_pipeline.rim    # 예제 3: OISC + I/O + LLM 파이프라인
│   └── guarded_program_block.rim     # 예제 4: 교주님 블록 및 LLM 차단 가드 블록
